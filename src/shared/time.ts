...
export function parseTimeInput(raw: string): ParseResult | null {
  ...
  if (localMatch) {
    ...
    const date = new Date(Number(yyyy), Number(mm) - 1, Number(dd), Number(hh), Number(mi), Number(ss), ms);
...
export function parseTimestamp(source: string): ParseResult | null {
  ...
  return {
    ...
    unit,
    date,
    precisionTail: unit === "microseconds" ? digits.slice(-3) : unit === "nanoseconds" ? digits.slice(-6) : undefined
  };
}
...
function toMilliseconds(value: string, unit: TimestampUnit): number | null {
  try {
    ...
    const divisor = unit === "seconds" ? 1n : unit === "milliseconds" ? 1n : unit === "microseconds" ? 1000n : 1000000n;
    const multiplier = unit === "seconds" ? 1000n : 1n;
    const millis = unit === "seconds" || unit === "milliseconds" ? bigint * multiplier : bigint / divisor;
