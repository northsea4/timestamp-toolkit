...
import { AlertTriangle, Command, Copy, History, Home, Search, Settings as SettingsIcon, Trash2, X } from "lucide-react";
import "../styles/app.css";
import { DEFAULT_SETTINGS, TIMEZONES } from "../shared/constants";
import { addHistory, clearHistory, loadHistory, loadSettings, saveSettings, takePendingSelection } from "../shared/storage";
...
function PopupApp() {
  ...
  const rows = useMemo(() => makeFormatRows(activeDate, settings.timezone), [activeDate, settings.timezone]);
...
function Header({ view, setView, openCommand }: { view: ViewName; setView: (view: ViewName) => void; openCommand: () => void }) {
  return (
...
function ConvertView({
  ...
  const timezoneLabel = TIMEZONES.find((timezone) => timezone.id === settings.timezone)?.label ?? settings.timezone;
  ...
  return (
    <main className={isError ? "convert-view parse-error" : isParseMode && parsed ? "convert-view parse-active" : "convert-view live-active"}>
      <section className={resultMode === "live" && hasInput ? "input-card input-muted" : isError ? "input-card input-error" : "input-card"}>
        ...
        <div className="input-meta">
          <span>{!hasInput ? "支持秒/毫秒/微秒/纳秒与日期时间" : parsed ? `识别为 ${timestampUnitLabel(parsed.unit)}` : "无法识别为时间戳或日期时间"}</span>
      ...
      <section className="result-toolbar">
        <div className={isError ? "mode-switch error" : "mode-switch"}>
          <button className={resultMode === "live" ? "active" : ""} type="button" onClick={() => setResultMode("live")}>
            实时
          </button>
          <button className={resultMode === "parse" ? "active" : ""} type="button" onClick={switchToParse}>
            解析
          </button>
        </div>
        ...
        <label className="timezone-pill">
          ...
          <select value={settings.timezone} onChange={(event) => void setSettings({ ...settings, timezone: event.target.value })}>
      ...
      <section className={isError ? "result-panel error" : isParseMode && parsed ? "result-panel parse" : "result-panel live"} aria-label="格式结果">
        {isError ? (
...
function FormatLine({ row, copyValue }: { row: FormatRow; copyValue: (value: string, label?: string) => Promise<void> }) {
  return (
    <div className={row.primary ? "format-line primary" : "format-line"}>
      ...
      <button className="copy-button" type="button" title={`复制${row.label}`} onClick={() => void copyValue(row.value, row.label)}>
        <Copy size={15} />
      </button>
    </div>
  );
...
function SettingsView({ settings, setSettings }: { settings: Settings; setSettings: (settings: Settings) => Promise<void> }) {
  return (
    <main className="panel-view">
...
function CommandPalette({
  ...
  const navCommands = [
    { key: "nav-convert", title: "打开转换", detail: "返回主界面", action: async () => setView("convert") },
    { key: "nav-recent", title: "打开历史", detail: "查看最近 30 条", action: async () => setView("recent") },
    { key: "nav-settings", title: "打开设置", detail: "主题、时区、剪贴板", action: async () => setView("settings") }
  ];
    ...
    <div className="command-backdrop" onMouseDown={close}>
      <div className="command-box" onMouseDown={(event) => event.stopPropagation()}>
        <div className="command-input">
          <Search size={17} />
          <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索格式、历史或设置" />
        </div>
...
function IconButton({
  ...
  return (
    <button className={active ? "icon-button active" : "icon-button"} type="button" title={title} onClick={onClick}>
      {children}
    </button>
  );
}
